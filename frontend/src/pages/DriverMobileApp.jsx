...
const DriverMobileApp = ({ user, profile: initialProfile }) => {
  ...
  const { 
    ...
    acceptCargo, 
    completeCargo 
  } = useCargos(user.id, profile.capacity_tons);
  ...
  const handleAcceptOrder = async (cargoId) => {
    await acceptCargo(cargoId);
    setDriverStatus('busy');
  };

  const handleCompleteDelivery = async () => {
    try {
      await completeCargo(currentDelivery?.id);
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          ...
          {availableCargos.map((cargo) => (
            <CargoCard
              key={cargo.id}
              cargo={cargo}
              onAccept={() => handleAcceptOrder(cargo.id)}
              showActions={true}
            />
          ))}
        </div>
