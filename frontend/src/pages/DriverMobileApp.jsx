...
const supabase = createClient(
  ...
  import.meta.env.VITE_SUPABASE_ANON_KEY || 'YOUR_SUPABASE_KEY'
);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-4">
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="text-right">
          <p className="font-bold text-lg text-blue-600">
            {cargo.expected_fare?.toLocaleString()}원
          </p>
          <p className="text-xs text-gray-500">
            {cargo.route_distance_km?.toFixed(1)}km
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center space-x-2">
          ...
          <p className="text-sm font-medium text-gray-900">{cargo.dms_orig_name}</p>
        </div>
        <div className="ml-1.5 border-l-2 border-dashed border-gray-300 h-4"></div>
        <div className="flex items-center space-x-2">
          <div className="w-3 h-3 rounded-full bg-red-500"></div>
          <p className="text-sm font-medium text-gray-900">{cargo.dms_dest_name}</p>
        </div>
      </div>
...
const DriverMobileApp = () => {
  ...
  // 사용 가능한 화물 목록 로드
  const loadAvailableCargos = async () => {
    try {
      const { data, error } = await supabase
        .from('cargo_data')
        .select('*')
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center">
            <p className="text-2xl font-bold text-blue-600">{myCargos.length}</p>
            <p className="text-sm text-gray-600">완료 배송</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-green-600">
              {myCargos.reduce((sum, c) => sum + (c.expected_fare || 0), 0).toLocaleString()}원
            </p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-red-600">-{commission.toLocaleString()}원</span>
            </div>
            <div className="flex justify-between font-medium pt-2 border-t border-gray-100">
              <span>실수령액</span>
              <span className="text-blue-600">{netEarnings.toLocaleString()}원</span>
            </div>
          </div>
        </div>
      </div>
